// components/forms/CourseUploadForm.tsx
"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { useDropzone } from "react-dropzone";
import { FileText, Upload, Loader2, CheckCircle2, XCircle } from "lucide-react";
...
export function CourseUploadForm() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      ...
      if (images.length > 6) {
        toast.error(
          "Vous ne pouvez pas télécharger plus de 6 images à la fois."
        );
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!courseName.trim()) {
      toast.error("Veuillez saisir un nom de cours");
      return;
    }
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: Primitives et équations différentielles"
              className="mt-1"
        ...
        >
          {isUploading ? (
            ...
            <>
              ...
              Enregistrer le cours ({files.length} fichier
              {files.length !== 1 ? "s" : ""})
            </>
          )}
        </Button>
